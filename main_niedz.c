...
//	   k=f_open(&fsrc,"0:car.rgb",FA_READ|FA_OPEN_EXISTING);
//
//	   bitmap_RGB();
//
//	   if(k==0)
...
//	   for(int i=0;i<240;i++)
//	   {
//		   k=f_read(&fsrc,buffer,960,&n);
//	   	   DMA_Config(960,buffer);
...
//	  f_opendir(&dir,"0:music");
	  k=f_open(&fsrc,"0:fink.mp3", FA_READ | FA_OPEN_EXISTING );
//	  k=f_open(&fsrc,"0:music/Bbagno.mp3", FA_READ | FA_OPEN_EXISTING );
	  if(k==0)LED_OFF;

//	  xTaskCreate(video_player,(char const*)"video_player",8192,NULL,tskIDLE_PRIORITY + 2,&video_Handle);
	  xTaskCreate(MP3_player,(char const*)"MP3_player",2048,NULL,tskIDLE_PRIORITY + 1,&MP3_Handle);
...
{
		int s1=2187;
	  f_open(&fsrc,"0:car.rgb", FA_READ | FA_OPEN_EXISTING );
	  ...
	  while(s1==2187)
	  {
		  f_read(&fsrc, buffer, BUF, &s1);
		  DMA_Config(BUF,buffer);
		  while(!(DMA2->LISR & DMA_LISR_TCIF0)){};
		  DMA2->LISR=0x00000000;
//		  read-=BUF;
